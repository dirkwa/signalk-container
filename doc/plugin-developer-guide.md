...
`cpus` is capped the same way. Docker validates a container's CPU cap against the daemon's core count at create **and** update — `cpus: 1.5` on a 1-vCPU Docker host fails with `range of CPUs is from 0.01 to 1.00, as there are only 1 CPUs available` (HTTP 400), and only the first time the container has to be (re)created, which is when it is hardest to see coming (podman accepts the value, but a cap above the core count grants nothing there either). signalk-container reads the daemon's `NCPU` from `/info` at runtime detection and lowers any higher `cpus` — from the plugin default or a user override — to it, on the ensureRunning path and on `updateResources`. The lowered value is what gets created, labelled and compared against the live container, so it is stable across reconciles. `EnsureRunningOptions.onResourceClamped` fires with `{ resource: "cpus", requested, granted, reason }` so you can surface it like `onUlimitClamped`; where the daemon does not report a CPU count nothing is clamped (as before). Requires signalk-container ≥ 1.28.1.
