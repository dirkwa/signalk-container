...
| Use case                                                                                                                                                                                                                                        | API                                    | Why                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mount **signalk-container's own data dir** — shared by every managed container — at a chosen container path. Namespace a subdirectory. Rejects a named volume attached to a parent directory, which would expose that volume's entire contents. | `ContainerConfig.signalkDataMount`     | Declarative — set the container path on the config object, signalk-container resolves the host side. Plugin sees the same string regardless of deployment.                                                                                                                                                                                                                                                                 |
...
| Mount a **specific path inside or outside** the data dir (a chart directory, an existing host download cache, etc.).                                                                                                                            | `containers.resolveHostPath(absPath)`  | General-purpose. Returns `{ source, subPath }`. A bind-backed path is narrowed to the exact host source with `subPath: ""`; only a **named volume** returns a non-empty `subPath`, because the compat API ignores a volume subpath so the volume arrives whole and the consumer must navigate from its root. Append `subPath` only when it is non-empty. `null` when SK is in a container and no SK mount covers the path. |
...
3. **Preserves per-plugin scoping** — `app.getDataDirPath()` in _your_ plugin resolves to `<configRoot>/plugin-config-data/<yourPluginId>` (Signal K rewrites it per-plugin), so translating your own path keeps the managed container scoped to your subdirectory. This is the difference from `signalkDataMount`, which resolves the same call against _signalk-container's_ app and therefore lands in its directory, shared by every managed container.
...
| Maintaining your own config-hash file to detect drift             | Hash file lost when the container owns the directory, and each plugin hashes a different field subset           | Delete it. `ensureRunning` compares the requested config against the live container on every call and recreates on drift — see [Removed: hash-file pattern](#container-config-changes)                                        |
| Raw `app.getDataDirPath()` in `volumes` when SK is in a container | Container fails to create with `Error: statfs <path>: no such file or directory` from the host's runtime daemon | Use `await containers.resolveHostPath(absPath)` to keep your own per-plugin directory, or `signalkDataMount` for shared scratch space, so signalk-container translates the SK-container-internal path back to the host source |
