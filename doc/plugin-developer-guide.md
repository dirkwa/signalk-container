...
| Mount a **specific path inside or outside** the data dir (a chart directory, an existing host download cache, etc.).                                                                                                                            | `containers.resolveHostPath(absPath)`  | General-purpose. Returns `{ source, subPath }`. A bind-backed path is narrowed to the exact host source with `subPath: ""`; only a **named volume** returns a non-empty `subPath`, because the volume arrives whole and the consumer must navigate from its root. Append `subPath` only when it is non-empty. `null` when SK is in a container and no SK mount covers the path. |
