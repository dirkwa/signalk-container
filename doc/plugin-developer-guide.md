...
When config drift needs a recreate but the runtime cannot stop or remove the live container ("operation not permitted" — under rootless Podman this is almost always an orphaned user namespace after a Podman service restart), signalk-container defers the recreate and fires `EnsureRunningOptions.onContainerWedged` **once** (not on every reconcile) with `{ name, drift, reason }`. The container cannot be recovered from the API — the `reason` names the remedy (`podman system migrate`, or kill its conmon/child and `podman rm -f`) — so surface it via `setPluginError` rather than `setPluginStatus`; recorded data is safe in the host bind mount. The advisory re-arms automatically once the container recovers. Requires signalk-container ≥ 1.29.0.
