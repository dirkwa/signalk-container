...
| GET    | `/containers/:name/logs?tail=N&since=ts` | Last N lines of the container's combined stdout+stderr log (one-shot). `tail` defaults 200, max 10000. When the container has produced no log output (e.g. it never started) and the runtime recorded a start failure, the response carries `lastError` (inspect `.State.Error`); absent when there is no recorded error |
