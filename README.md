...
| `recreate(name, config, options?)`              | Force-recreate: remove (if live) + ensureRunning. Never short-circuits — use this for "update now" / plugin-startup self-heal flows where correctness must not depend on drift detection (1.12.0+)                                                                 |
