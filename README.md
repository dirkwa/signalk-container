...
- **Host device access** -- `ContainerConfig.devices` exposes host device nodes or whole device directories (`/dev/snd`, `/dev/input`, …) to a managed container, hot-plug-safe so a USB replug keeps working, and `ContainerConfig.groupAdd` grants the host groups that own those nodes. Works across docker and rootless Podman; a missing device is skipped with a warning rather than blocking start. See [Exposing host devices](#exposing-host-devices-devices-groupadd).
...
On rootless Podman (the recommended deployment) device access is governed by plain file permissions on the bound nodes rather than device-cgroup rules, so `groupAdd` is what actually grants access: `groupAdd: ["audio"]` works as long as the user running Podman is in the host `audio` group. signalk-container handles the rootless user-namespace details for you so that host group membership reaches the container process.
