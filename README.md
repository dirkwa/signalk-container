...
| `cleanupOrphanedJobs(filter)`                   | Reap leftover one-shot job containers tagged with the caller's `ownerPluginId`. Pass an `expectedJobNames` allowlist to spare in-flight jobs. Returns counts and the list of removed names                                                                         |
