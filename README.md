...
- **Healthcheck handling** -- an image's own `HEALTHCHECK` is re-emitted as explicit run flags so it works across runtimes (Podman drops image-healthcheck inheritance over the Docker-compat socket and parks the container in `starting`). For images that ship **no** healthcheck, consumer plugins set `ContainerConfig.healthcheck` to supply a probe — otherwise Podman reports them as `starting` forever. See the [developer guide](doc/plugin-developer-guide.md#healthcheck-for-images-that-ship-none).
