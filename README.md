...
- **CPU caps that fit the host** -- a `cpus` limit above the daemon's core count is lowered to it instead of failing the container create (Docker rejects `--cpus 1.5` on a 1-vCPU host outright); the consumer plugin is told via `onResourceClamped`. See the [developer guide](doc/plugin-developer-guide.md#resource-limits).
