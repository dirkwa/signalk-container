...
- **crashLooping keys on a RATE, never a raw count.** `RestartCount` from inspect is a _lifetime_ total and is legitimately non-zero on a healthy container under `--restart=unless-stopped` after a host reboot — a threshold on the raw value alerts on uptime, not on failure. `observeRestarts(name, count, now?)` keeps a per-container `{count, at}` baseline and raises only when the delta reaches `CRASH_LOOP_RESTARTS` (3) inside `CRASH_LOOP_WINDOW_MS` (5 min). The **first** observation of a container never raises (its history is unwitnessed), a _decreasing_ count re-baselines (the container was recreated), and a raise advances the baseline so the alert cannot outlive the loop. Driven by one shared `RESTART_POLL_MS` sweep over `lastConfigs` in `src/index.ts`, not a per-container timer, and deliberately **not** gated on `options.healthCheck`: a container dying at startup never reaches the state where a health check could run, so `pollHealth` cannot see this class of failure at all.
