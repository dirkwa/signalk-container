...
- **crashLooping keys on a RATE, never a raw count.** `RestartCount` from inspect is a _lifetime_ total and is legitimately non-zero on a healthy container under `--restart=unless-stopped` after a host reboot — a threshold on the raw value alerts on uptime, not on failure. `observeRestarts(name, count, now?)` keeps per-container timestamped samples over a **sliding** `CRASH_LOOP_WINDOW_MS` (5 min) and raises when the count grows by `CRASH_LOOP_RESTARTS` (3) across it. The window must slide, not tumble: a single baseline that resets on expiry splits a burst straddling the boundary (one restart before, two after = three in two minutes) and misses it. Pruning keeps the newest already-aged-out sample, which is the only record of the count when the window opened. When even the sample _before_ that one predates the window, nothing was observed inside it — the poll stalled — so it re-anchors instead of alerting: the restarts are real but undatable, and charging them to the last five minutes reports a rate never observed. `sweepRestartsOnce` is exported from `src/index.ts` with injected deps so the orchestration (failure isolation, retire-on-stop, vanished containers, set snapshotting) is testable at the function boundary. The **first** observation of a container never raises (its history is unwitnessed), a _decreasing_ count re-baselines (the container was recreated), and the alert clears as soon as the windowed delta falls back under the threshold, so it cannot outlive the loop. Driven by one shared `RESTART_POLL_MS` sweep over `lastConfigs` in `src/index.ts`, not a per-container timer, and deliberately **not** gated on `options.healthCheck`: a container dying at startup never reaches the state where a health check could run, so `pollHealth` cannot see this class of failure at all.
