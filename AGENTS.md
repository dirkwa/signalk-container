...
- **NO `method`.** The emitter states severity only; the NotificationManager owns presentation (RFC notification-handling §6.1). The `raise` option type deliberately omits `method` so the `updates/service.ts` legacy `handleMessage`+`method:["visual"]` anti-pattern can't be copied. (`updates/service.ts` is the known legacy emitter, slated to migrate later — separate PR.)
